...
public class Main extends JFrame {
	...
	private void setupJComponents() {
		...
		file = new JMenu("File");
		file.getAccessibleContext().setAccessibleDescription("Open file");
		menuBar.add(file);
	...
	private void setupStatSliders() {
		for (int i = 0; i < statSliders.length; i++) {
			...
			statSliders[i] = new JSlider(JSlider.HORIZONTAL, STAT_MIN[i], STAT_MAX[i], STAT_INIT[i]);
	...
	private void changeValue(JSlider slider) {
		for (int i = 0; i < statSliders.length; i++) {
			if (slider.equals(statSliders[i]))
				stats[i] = slider.getValue();
		}
	}
	...
	private void saveAndExportFile() {
		...
		// } catch (IOException e) {
		// message = "ERROR: Could not save as there are not enough source
		// files";
		// e.printStackTrace();
		// }
